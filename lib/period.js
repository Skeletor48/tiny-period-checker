exports.calculateTouchPointType = clientData => {
  if (
    ...
    (clientData.ClientSegment && clientData.ClientSegment.frequency === 'None')
  ) {
    return 'None'
  }
  if (
    !clientData.clientTouchedAt &&
    clientData.ClientSegment.frequency !== 'None'
  ) {
    return 'Upcoming'
  }
  const frequency = clientData.ClientSegment.frequency
  ...
  return getTouchPointType(touchPoint, frequency)
}

const getTouchPointType = (touchPoint, frequency) => {
  const isTouchedInCurrentPeriod = periodChecker(frequency, touchPoint, true)

  if (isTouchedInCurrentPeriod) {
    return 'Done'
  }

  const isTouchedInPreviousPeriod = periodChecker(frequency, touchPoint)
...
const periodChecker = (frequency, touchPoint, isCurrentPeriodCheck) => {
...
const getPeriod = (frequency, date) => {
